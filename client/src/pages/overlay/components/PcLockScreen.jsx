...
export default function PcLockScreen() {
  ...
  useEffect(() => {
    if (walkinDeclineEvent && walkinRequested) {
      showToast(walkinDeclineEvent.reason || 'Operator declined the request', 'error');
      setWalkinRequested(false);
      setIsRequestingWalkin(false);
      setStep('selection'); // Optionally return to main menu
    }
  }, [walkinDeclineEvent, walkinRequested, showToast]);
  ...
  const handleStartSession = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Fetch the active session from the backend so the lock screen unmounts
        await fetchSession();
      } else {
  ...
  const handleWalkinRequest = async () => {
    ...
    try {
      const res = await requestWalkinSession(walkinName.trim(), durationMinutes);
      if (res?.success) {
        setWalkinRequested(true);
        showToast('Request sent to operator!', 'success');
      } else {
        showToast(res?.error || 'Failed to send request', 'error');
      }
    } catch (err) {
      showToast('Error communicating with operator', 'error');
    } finally {
      setIsRequestingWalkin(false);
    }
  };
