              ...
              {foodItems.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    <span className="text-text-3 font-mono">{item.quantity}x</span>
                    <span className="text-text-2 font-body">{item.name}</span>
                  </div>
                  <span className="text-text font-mono font-semibold">₹{(item.price * item.quantity).toFixed(2)}</span>
                </div>
              ))}
            </div>
          )}
