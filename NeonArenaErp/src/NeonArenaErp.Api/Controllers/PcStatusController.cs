...
public class PcsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetDetails([FromQuery] Guid? branchId = null)
    {
        ...
        var pcs = await _unitOfWork.Repository<NeonArenaErp.Domain.Entities.Pc>()
            .Query()
    ...
    [HttpPost]
    [Authorize(Policy = "SuperAdminOnly")]
    public async Task<IActionResult> Create([FromBody] NeonArenaErp.Application.DTOs.Settings.CreatePcDto dto)
    {
        var exists = await _unitOfWork.Repository<NeonArenaErp.Domain.Entities.Pc>()
            .Query()
        ...
        var pricingProfile = await _unitOfWork.Repository<NeonArenaErp.Domain.Entities.PricingProfile>()
            .Query()
            .FirstOrDefaultAsync(p => p.BranchId == dto.BranchId && p.IsActive);

        var pc = new NeonArenaErp.Domain.Entities.Pc
        {
            Id = Guid.NewGuid(),
            ...
            State = NeonArenaErp.Domain.Enums.PcState.Idle,
            IsActive = true,
            IsDeleted = false,
        ...
        await _unitOfWork.Repository<NeonArenaErp.Domain.Entities.Pc>().AddAsync(pc);
    ...
    [HttpPut("{id}")]
    [Authorize(Policy = "SuperAdminOnly")]
    public async Task<IActionResult> Update(Guid id, [FromBody] NeonArenaErp.Application.DTOs.Settings.UpdatePcDto dto)
    {
        var pc = await _unitOfWork.Repository<NeonArenaErp.Domain.Entities.Pc>()
            .Query()
        ...
        _unitOfWork.Repository<NeonArenaErp.Domain.Entities.Pc>().Update(pc);
        await _unitOfWork.SaveChangesAsync();
