...
using NeonArenaErp.Api.Middleware;
...
using NeonArenaErp.Infrastructure.Services;
using Serilog;

// ═══════════════════════════════════════════════
//  NeonArena ERP — .NET 8 Enterprise Backend
...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File("logs/neonarena-.log", 
...
{
    options.AddPolicy("NeonArenaCors", policy =>
    {
        policy
...
if (!string.IsNullOrEmpty(redisConnectionString))
{
    ...
    {
        options.Configuration.ChannelPrefix = StackExchange.Redis.RedisChannel.Literal("NeonArenaSignalR");
    });
}

// ── 7. FluentValidation ──
builder.Services.AddValidatorsFromAssemblyContaining<NeonArenaErp.Application.Validators.Auth.AdminLoginValidator>();
...
builder.Services.AddSingleton(sp =>
{
    ...
    return new JwtTokenService(
        ...
        config["Issuer"] ?? "NeonArenaErp", config["Audience"] ?? "NeonArenaErpClient"
    );
});
...
builder.Services.AddScoped<IHubNotificationService, NeonArenaErp.Api.Services.HubNotificationService>();
...
{
    c.SwaggerDoc("v1", new() { Title = "NeonArena ERP API", Version = "v2.0" });
...
{
    app.UseSwagger();
    app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "NeonArena ERP API v2.0"));
}

// ── 2. CORS (maps from Helmet + CORS) ──
app.UseCors("NeonArenaCors");
...
{
    Log.Information("╔═══════════════════════════════════════════════╗");
    Log.Information("║   NeonArena ERP — .NET 8 Enterprise Backend  ║");
